...
def update_profile(
    identity_id=None, name=None, email=None, institution=None, role=None
):
...
def logout():
    ...
    app.logger.info(
        f"{session['name']} \
        ({session['primary_identity']}) logged out"
    )
    session.clear()
...
def profile():
    ...
    if profile:
        ...
        create_profile(
            identity_id=identity_id, name=session["name"], email=session["email"]
        )
...
def authcallback():
    ...
    if "code" not in request.args:
        ...
        if profile:
            ...
            app.logger.info(
                f"Successful login by {profile.name} \
              (Globus identity: {profile.identity_id})"
            )
